...
module "Hub-ExtWAF-NSG" {
  ...
  # Needs to match with the one assigned in Prod-Hub.subnet_nsg.
  subnet_id_list = [module.Prod-Hub.subnets[local.ExtWAFSubnet]]
}
...
module "Hub-IntWAF-NSG" {
  ...
  # Needs to match with the one assigned in Prod-Hub.subnet_nsg.
  subnet_id_list = [module.Prod-Hub.subnets[local.IntWAFSubnet]]
}
...
module "Hub-Net01-NSG" {
  ...
  # Needs to match with the one assigned in Prod-Hub.subnet_nsg.
  subnet_id_list = [module.Prod-Hub.subnets[local.HubNet01]]
}

module "Hub-WAD-Net01-NSG" {
  source         = "../modules/network/nsgSet"
  resource_group = module.Prod-Hub.resource_group
  nsg_name       = var.Hub-WAD-Net01-NSG
  nsg_rule_set   = var.Hub-WAD-Net01-NSG-Rules
  # Needs to match with the one assigned in Prod-Hub.subnet_nsg.
  subnet_id_list = [module.Prod-Hub.subnets[local.HubWADNet01]]
}
...
module "Hub-Az-FW01" {
  ...
  subnet_id               = module.Prod-Hub.subnets[local.FirewallSubnet]
...
module "Hub-S2S-VPN" {
  ...
  gw_subnet_id    = module.Prod-Hub.subnets[local.GatewaySubnet]
...
module "Hub-Ext-DMZ-APPGW01" {
  ...
  resource_group             = module.Prod-Hub.resource_group
  appgw_subnet_id            = module.Prod-Hub.subnets[local.ExtWAFSubnet]
  appgw_pip                  = var.Hub-Ext-DMZ-APPGW01-PIP01
  application_gateway_config = var.Hub-Ext-DMZ-APPGW01
}

module "Hub-Int-DMZ-APPGW01" {
  source                     = "../modules/network/appGateway"
  resource_group             = module.Prod-Hub.resource_group
  appgw_subnet_id            = module.Prod-Hub.subnets[local.IntWAFSubnet]
  appgw_pip                  = var.Hub-Int-DMZ-APPGW01-PIP01
  application_gateway_config = var.Hub-Int-DMZ-APPGW01
}
